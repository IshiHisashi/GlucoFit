...
const medicineLogResolvers = {
  Query: {
    ...
    getMedicineLogsByUser: async (_: any, { user_id }: { user_id: string }) => {
      try {
        ...
        if (medicineLogs.some(log => log.medicine_id === null)) {
          throw new Error("Some medicine logs have an invalid or missing medicine_id reference.");
        }
    
        return medicineLogs;
      } catch (error) {
        console.error("Error fetching medicine logs for user:", error);
        throw new Error("Failed to fetch medicine logs. Please check if all references are correct.");
      }
    },
    ...
    getUserMedicineList: async (_: any, { user_id }: { user_id: string }) => {
      try {
        ...
        const userMedicineList = await UserMedicineList.find({ user_id: new Types.ObjectId(user_id) });
  ...
  Mutation: {
    ...
    createMedicineLog: async (
      _: any,
      { user_id, medicine_id, amount, log_timestamp }: { user_id: string; medicine_id: string; amount: number; log_timestamp: Date }
    ) => {
      // Validate if medicine_id exists in UserMedicineList
      const medicine = await UserMedicineList.findById(medicine_id);
      if (!medicine) {
        throw new Error("Invalid medicine_id: No matching medicine found in UserMedicineList.");
      }
    ...
    updateMedicineLog: async (
      ...
      return await MedicineLog.findByIdAndUpdate(id, args, { new: true }).populate("medicine_id user_id");
    },
